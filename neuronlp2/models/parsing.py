...
class BiRecurrentConvBiAffine(nn.Module):
    ...
    def _write_the_output(self, path, word, arc_dep, arc_head, lstm_out, rel_dep, rel_head):
        ...
        f = open(f_arc_dep, 'a')
        f.write(word+' '+' '.join([str(a) for a in arc_dep])+'\n')
        f.close()

        f = open(f_arc_head, 'a')
        f.write(word+' '+' '.join([str(a) for a in arc_head])+'\n')
        f.close()

        f = open(f_lstm_out, 'a')
        f.write(word+' '+' '.join([str(a) for a in lstm_out])+'\n')
        f.close()

        f = open(f_rel_dep, 'a')
        f.write(word+' '+' '.join([str(a) for a in rel_dep])+'\n')
        f.close()
        
        f = open(f_rel_head, 'a')
        f.write(word+' '+' '.join([str(a) for a in rel_head])+'\n')
        f.close()

    def get_syntax_feature(self, original_words, input_word, input_char, input_pos, mask=None, length=None, hx=None):
    ...
    def _get_rnn_output(self, original_words, input_word, input_char, input_pos, mask=None, length=None, hx=None):
        # [batch, length, word_dim]
        word = self.word_embedd(input_word)
        # apply dropout on input
        word = self.dropout_in(word)
    ...
    def forward(self, original_words, input_word, input_char, input_pos, mask=None, length=None, hx=None):
        # output from rnn [batch, length, tag_space]
        arc, type, _, mask, length = self._get_rnn_output(original_words, input_word, input_char, input_pos, mask=mask, length=length, hx=hx)
        # [batch, length, length]
    ...
    def decode_mst(self, original_words, input_word, input_char, input_pos, mask=None, length=None, hx=None, leading_symbolic=0):
        '''
        Args:
            input_word: Tensor
        ...
        '''
        # out_arc shape [batch, length, length]
        out_arc, out_type, mask, length = self.forward(original_words, input_word, input_char, input_pos, mask=mask, length=length, hx=hx )

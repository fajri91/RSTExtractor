...
def read_data_for_test(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet, max_size=None,
                          normalize_digits=True, symbolic_root=False, symbolic_end=False,
                          use_gpu=False, volatile=False):
    reader = CoNLLXReader(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet)
    print('Reading data from %s' % source_path)
    inst = reader.getNextForTest(normalize_digits=normalize_digits, symbolic_root=symbolic_root, symbolic_end=symbolic_end)
    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        for bucket_size in _buckets:
            if inst_size < bucket_size:
                bucket = bucket_size
                data = [sent.word_ids, sent.char_id_seqs, inst.pos_ids]
                max_len = max([len(char_seq) for char_seq in sent.char_seqs])
                break
        ...
        index = slice(0,1)
        ret_value.append((words[index], chars[index], pos[index], masks[index], lengths[index], sent.words))
        inst = reader.getNextForTest(normalize_digits=normalize_digits, symbolic_root=symbolic_root, symbolic_end=symbolic_end)
    reader.close()
    return ret_value
